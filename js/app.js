...
async function loadDashboard() {
    ...
    try {
        ...
        const { data: timeEntries } = await window.appConfig.supabaseClient.from('time_entries').select('*');
        const { data: activities } = await window.appConfig.supabaseClient.from('activities').select('*').order('created_at', { ascending: false }).limit(10);
        ...
        await renderActiveProjects(activeCases);
        ...
        // 统计数据
        document.getElementById('stat-cases').textContent = visibleCases.length;
...
// 渲染进行中的项目
async function renderActiveProjects(cases) {
            ...
            <div class="project-card cursor-pointer" onclick="showCaseDetail('${caseItem.id}')">
                <div class="p-4 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs px-2 py-1 bg-${color}-50 text-${color}-600 rounded-full">${caseItem.type || '其他'}</span>
                    </div>
                </div>
                <div class="p-4">
        ...
        return `
            <div class="activity-item">
                ...
                <div class="flex-1">
                    ...
                    <div class="text-xs text-gray-400 mt-1">${act.user_name || '系统'} · ${window.appConfig.formatRelativeTime(act.created_at)}</div>
                </div>
            </div>
        `;
    }).join('');
