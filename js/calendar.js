...
function renderCalendar() {
    ...
    // 获取下月补充天数
    const nextMonthDays = (7 - ((startWeekday + totalDays) % 7)) % 7;
    if (nextMonthDays === 0) {
        // 如果正好填满，不需要补充
    }
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        // 获取当天的事件
        const events = getEventsForDate(dateStr, cases, stages, tasks);
    ...
    // 下月日期
    const remainingCells = 42 - (prevMonthDays + totalDays); // 6行 × 7天 = 42格
...
function getEventsForDate(dateStr, cases, stages, tasks) {
    ...
    stages.forEach(stage => {
        if (stage.due_date && stage.due_date.startsWith(dateStr)) {
            const caseItem = visibleCases.find(c => c.id === stage.case_id);
            if (caseItem) {
                events.push({
                    type: 'project',
